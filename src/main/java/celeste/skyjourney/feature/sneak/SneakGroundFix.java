...
public class SneakGroundFix {
    // celeste作の足元にアンカーをおいた方式
    public static class SmartAnchor {
        /**
        ...
        public static Vec3d adjustMovement(Entity entity, Vec3d movement) {
            ...
            if (!hasSupport(world, localPos.x, localPos.y, localPos.z, entity, ship)) {
                return movement;
            }

            // X方向の移動安全確認
            double nextLocalX = localPos.x + localMove.x;
            boolean safeX = hasSupport(world, nextLocalX, localPos.y, localPos.z, entity, ship);
            if (!safeX) {
                localMove.x = 0;
            }
            ...
            boolean safeZ = hasSupport(world, localPos.x, localPos.y, nextLocalZ, entity, ship);
            if (!safeZ) {
                localMove.z = 0;
            }
            ...
            if (localMove.x != 0 && localMove.z != 0) {
                ...
                boolean safeDiag = hasSupport(world, diagX, localPos.y, diagZ, entity, ship);
                if (!safeDiag) {
                    localMove.x = 0;
                    localMove.z = 0;
                }
         ...
         */
        private static boolean hasSupport(World world, double localX, double localY, double localZ, Entity entity, Ship ship) {
            // 足元領域の定義
            ...
            return hitBlock; // 足場が存在するかを返す
        }
    }

    // iruru作の四隅にアンカーをおいた方式
    public static class CrossAnchor {
        ...
        private static boolean isAir(World world, double localX, double localY, double localZ, double minXOffset, double maxXOffset, double minZOffset, double maxZOffset, Entity entity, Ship ship) {
            ...
            return !hitBlock; // ブロックがあるということはairではないので反転して返す
        }
    }
